...
**发版与 Git 无关**：Git 只做源码维护；上线为 **本机构建 → 上传产物 → 服务器解压、`npm install --omit=dev`、`systemctl restart`**（详见 **[`deploy/RUNBOOK.md`](./deploy/RUNBOOK.md) §3.0**，§8 速查）。门户打包：**[`deploy/sync-web.sh`](./deploy/sync-web.sh)**；可选 `--push`。
