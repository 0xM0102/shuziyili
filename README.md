...
**发版方式（当前唯一）**：本机构建 → `scp` 上传产物 → 服务器解压、`npm install --omit=dev`、`systemctl restart`（详见 **[`deploy/RUNBOOK.md`](./deploy/RUNBOOK.md) §3**）。门户打包：**[`deploy/sync-web.sh`](./deploy/sync-web.sh)**（仅打 tgz，上传见 RUNBOOK）。
