...
/** 头像 URL 变化时由父级 key 重置挂载，无需在 effect 里 setState。 */
function AvatarFace({ rawUrl, initial, className, asDiv, ariaLabel, onNavigate }: FaceProps) {
  const canTryImg = isHttpAvatar(rawUrl);
  const [imgFailed, setImgFailed] = useState(false);
  const showPhoto = canTryImg && !imgFailed;
